...
let hash = ref()
let route = useRoute()
let selectedHash = ref()

onMounted(() => {
    console.log(route.hash)
    if (!route.hash)
        selectedHash.value = "#0"
    else
        selectedHash.value = route.hash
})
watch(route, async (newHash, oldHash) => {
    // console.log(newHash.hash, oldHash.hash)
    selectedHash.value = newHash.hash || "#0"
